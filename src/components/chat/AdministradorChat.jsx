...
const AdministradorChat = ({ socket }) => {
  ...
  <li key={index} className="flex items-center justify-between">
    <span onClick={() => handleClickRoom(salasClientes[sala])} className={`cursor-pointer text-xl ${selectedRoom === salasClientes[sala] ? 'bg-d' : ''} hover:bg-gray-100`}>
      Room: {salasClientes[sala]}
      {newMessagesCount[salasClientes[sala]] > 0 && (
        <span className="relative inline-block">
          <FaBell />
          <span className="absolute top-0 right-0 bg-red-500 text-white rounded-full px-2 text-xs transform translate-x-1/2 -translate-y-1/2">{newMessagesCount[salasClientes[sala]]}</span>
        </span>
      )}
    </span>
