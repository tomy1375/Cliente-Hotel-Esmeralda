...
const simulatedEmails = ["tomy_ramos1991@yahoo.com.ar", "tomy1375"];
...
function ForgotPasswordSubheader() {
  return (
...
function RememberPasswordSection() {
  ...
  const handleContinueClick = () => {
    ...
    } else if (simulatedEmails.includes(emailInput)) {
      Swal.fire({
        title: 'Success!',
        text: `Please check the email address ${emailInput} for instructions to reset your password.`,
        icon: 'success',
        confirmButtonText: 'Ok',
        confirmButtonColor: '#fcd34d',
        customClass: {
        ...
        // Navigate to "/forgotPasswordRecovery" after successful email check
        navigate("/forgotPasswordRecovery", { state: { email: emailInput } });
      });
    } else {
      Swal.fire({
        title: 'Error!',
        text: `The email ${emailInput} is not registered.`,
        icon: 'error',
        confirmButtonText: 'Ok',
        confirmButtonColor: '#fcd34d',
        customClass: {
  ...
  return (
    <div className="flex gap-4 self-end mt-12 text-2xl font-bold tracking-tighter max-md:flex-wrap max-md:mt-10">
      <div className="flex-auto my-auto leading-8 text-right text-white">
        ...
        <br />
        <button onClick={handleLoginClick} className="hover:text-amber-400 transition-colors">
          <span className="">Back to </span>login
        </button>
      </div>
      <button onClick={handleContinueClick} className="justify-center px-10 py-8 text-center whitespace-nowrap bg-amber-300 rounded-xl shadow-sm text-neutral-800 max-md:px-5  hover:bg-amber-400 transition-colors">
        Continue
      </button>
    </div>
  );
