        ...
        <div className="grid grid-cols-3 gap-11 mt-5 mb-9">
          {currentPageData.map((item, index) => (
            <div key={index} className="flex flex-col items-center">
              <img src={item.Url} alt={item.name} style={{ width: '500px', height: '400px' }} className="object-cover mb-7 overflow-auto cursor-pointer" onClick={() => openModal(item.Url)} />
              <p className="text-center text-3xl">{item.name}</p>
              <div className="overflow-auto text-center text-lg max-h-[100px]">
                {item.description}
              </div>
        ...
        <>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50">
            <div className="bg-white p-4 rounded-lg" style={{ maxWidth: '90%', maxHeight: '90%' }}>
              <img src={selectedImage} alt="Selected" className="w-full h-auto object-contain max-w-full max-h-full" />
              <button onClick={closeModal} className="mt-4 bg-d text-white px-4 py-2 rounded">Cerrar</button>
            </div>
          </div>
        </>
      )}
      {/* Fin Modal */}
      <div style={{ position: 'absolute', bottom: '-35px', left: '50%', transform: 'translateX(-50%)', backgroundColor: 'white', zIndex: 10 }} className="flex justify-center items-center mt-10">
